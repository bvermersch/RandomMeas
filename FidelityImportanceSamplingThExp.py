...
for iu in range(Nu):
    ...
    for iq in range(N):
        u[iu][iq] = SingleQubitRotation(random_gen,mode)
...
for iu in range(Nu):
    ...
    Prob1 = Simulate_Meas_pseudopure(N, GHZ_state, p_depo1, u[iu])
    Prob1 = np.reshape(Prob1, [2]*N)
    X_uni_1[iu] = get_X(Prob1,N)
    prob2e = get_prob(Meas_Data_uni_2[iu,:], N)]
    x_uni_2[iu] = get_x(prob2e,n)
    X_uni_fidelity[iu] = get_X_overlap(Prob1,prob2e,N)]

p2_uni_1 = 0
p2_uni_2 = 0 
RM_fidelity_uni = 0

p2_uni_1 = np.mean(X_uni_1)
