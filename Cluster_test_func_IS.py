...
def main(inp):
    ...
    N, nu, nm, p, nexp = I['N'], I['nu'], I['nm'], I['p'], I['exps']
    ...
    def RM_sn (NN,num_nu, num_nm, var_theta, var_phi):
        ...
        for iu in range(num_nu):
            ...
            probb /= sum(probb)
            """
            rho_rm = Qobj(rho_exp, [[2]*NN,[2]*NN])
            u = [0]*NN
            for iq in range(NN):
                u[iq] = RY(np.arcsin(np.sqrt(var_theta[iq, iu]))*2)*RZ(2*math.pi*var_phi[iq, iu])
            uf = tensor(u)
            rho_u = uf*rho_rm*uf.dag()
            #print(rho_u)
            probb_test = np.real(np.diag(rho_u.full()))
            print(probb)
            print(probb_test)
            """
            prob_tens = np.zeros([2]*NN) 
    ...
    #nm = 1000# number of readout measurements per unitary
    ...
    S = {}
    S['Error_is'] = Error_is
    S['Error_uni'] = Error_uni
    S['p2_is'] = p2_IS
    S['p2_uni'] = p2_uni
    
    
    fout1 = open(inp+'.out','wb')
    pickle.dump(S, fout1)
    fout1.close()
    fin.close()
