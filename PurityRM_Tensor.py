...
## Parameters
N = 16 # Number of qubits to analyze (necessary <= 16 for the present code due to the use of the function np.unpackbits)
if (N>16):
    print('Please reduce N (or adapt the call to np.unpackbits)')
Nu = 4000 # Number of random unitaries to be used
NM = 100000 # Number of projective measurements (shots) per random unitary
mode = 'xyz'
...
for iu in range(Nu):
        ...
        Unitary = [0]*N
        for i in range(N):
            Unitary[i] = SingleQubitRotation(random_gen,mode)

        #Apply on state and compute bitstrings probabilities
        Co_op = Unitary+[psi]
        #if iu==0:
            #path_info = np.einsum(Co,*Co_op,optimize=True)
        psiu = np.einsum(Co,*Co_op,optimize='greedy').flatten()
#path_in#fo = np.einsum_path('ac,c->a',np.zeros((NM,N)), np.zeros(N),optimize='greedy')
        ##psiu = np.dot(Unitary,psi)
        Prob_bitstrings = np.abs(psiu)**2
...
#import qutip as qu
#Prob = 0*qu.tensor([qu.basis(2,0)]*N)
X = 0
for iu in range(Nu):
    Prob = np.zeros(tuple([2]*N))
    s = np.unpackbits(np.array(Meas_Data[iu,:], dtype=">i2").view(np.uint8)).reshape((NM,16))[:,-N:]
    for iM in range(NM):
        Prob[tuple(s[iM])] += 1
    A = np.zeros((2,2))
    A[0,0] = 1
    ...
    A[1,0] = -0.5
    A[0,1] = -0.5
    #V = np.einsum('abc,aA,bB,cC->ABC',Prob,A,A,A)
    List = [Prob]+[A]*N+[Prob]
    Co = alphabet[:N]
    for i in range(N):
        Co += ','
    ...
    Co += alphabet_cap[:N]
    print(iu)
    X += 2**N*np.einsum(Co,*List,optimize='greedy')/NM/(NM-1)/Nu-2**N/(NM-1)/Nu
print(X)
##X = np.einsum('abc,abc',Prob,V)
##N = 16
##A = [0]*N
##l = [qu.qeye(2)]*N
##for i in range(N):
##    l[i] = qu.qeye(2)-qu.sigmax()
##    A[i] = qu.tensor(l)
#
...
##    print(Purity[i]*Nu/NM**2)
##    #Remove statistical bias
##    Purity[i] = Purity[i]/(NM*(NM-2))-2**N_Partition[i]/(NM-1)
##
